...
(function() {
    ...
    angular
      ...
      .component('smallLabel', {
          transclude: true,
          template:'<div ng-show="$ctrl.componentLabel == \'participant\'" class="small-label"><div class="small-label-text"><h3>{{$ctrl.baseInfo.participant_name}}</h3><span>CQ:{{$ctrl.baseInfo.cq_group}}</span><br></div><svg id="participant" class="barcode"></svg></div><div ng-show="$ctrl.componentLabel == \'biomaterial\'" layout="row" style="width: 41.27mm; height: 15.87mm; margin-bottom: 3mm !important; margin-left: 0.625mm; margin-right: 0.625mm; position: relative; padding: 2px; line-height: 14px;"><div style="width: 25.4mm; background: #3883ff;"><div><span>{{$ctrl.material.label}}</span></div><div ng-style=\'{"overflow": "hidden","white-space": "nowrap","text-overflow": "ellipsis"}\'><span>{{$ctrl.baseInfo.participant_name}}</span><br></div><div layout="row" flex><span>NR:{{$ctrl.baseInfo.recruitment_number}}</span></div><svg id="biomaterial" class="barcode"></svg></div><div style="width: 15.87mm; background: red;">here</div></div><div ng-show="$ctrl.componentLabel == \'unattached\'" class="small-label"><div class="small-label-text"><p>Laboratório</p><span>Centro:{{$ctrl.baseInfo.laboratoryFieldCenter}}</span><br><span>CQ:{{$ctrl.baseInfo.cq_group}}</span><br></div><svg id="unattached" class="barcode"></svg></div>',
          controller: Controller,
          bindings: {
            componentLabel: "<",
            base: '<',
...
(function() {
  ...
  angular
    ...
    .component('biomaterialLabel', {
      transclude: true,
      template:'<div><small-label ng-repeat="biomaterial in $ctrl.biomaterialList" biomaterial="biomaterial" base="$ctrl.baseInfo" component-label="\'biomaterial\'"></small-label></div>',
      controller: Controller,
      bindings: {
        base: '<',
        biomaterialList: '<'
...
(function() {
  ...
  function directive($window, $compile, LabelService) {
    var ddo = {
      ...
      scope: {},
      transclude: true,
      template:'<div id="print-page" layout-align="start center" ng-if="$ctrl.loadComponents"><h1>here</h1><biomaterial-label base="$ctrl.baseInfo" biomaterial-list="$ctrl.bioMaterialList"></biomaterial-label></div>',
      controller: Controller,
      controllerAs: '$ctrl',
    };
