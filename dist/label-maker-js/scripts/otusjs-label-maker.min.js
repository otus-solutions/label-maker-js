...
(function() {
  ...
  angular
    ...
    .component('tubesLabel', {
      transclude: true,
      template:'<div class="label"><div class="label-text"><h3>{{$ctrl.tube.label}}</h3><span>{{$ctrl.base.participant_name}}</span><br><span>NR:{{$ctrl.base.recruitment_number}} Sexo:{{$ctrl.base.gender}}</span><br><span>DN:{{$ctrl.base.birthday}}</span><br></div><svg id="{{$ctrl.tube.code}}" class="barcode"></svg></div>',
      controller: Controller,
      bindings: {
        base: '<',
        tube: '<'
